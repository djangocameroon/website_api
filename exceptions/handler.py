...
def api_exception_handler(exc, context):
    ...
    if isinstance(exc, RestException):
        return Response({'status': exc.status_code, 'message': exc.detail, 'errors': errors}, exc.status_code)
    ...
    if response is not None:
        ...
        return Response({'status': response.status_code,'message': message, 'errors': errors}, response.status_code)
